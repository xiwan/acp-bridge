...
def get_best_fallback(
    ...
    """Pick the best fallback agent using live pool state + historical stats.
    ...
    def score(agent: str) -> float:
        ...
        val = (base - trend_penalty) * (1.5 if has_idle else 1.0)
        log.debug("fallback_score: agent=%s score=%.1f (idle=%s rate_1h=%.2f rate_15m=%.2f dur=%.1f penalty=%.1f)",
                  agent, val, has_idle, rate_1h, rate_15m, avg_dur, trend_penalty)
        return val
...
def make_acp_agent_handler(agent_name: str, pool: AcpProcessPool, profile: dict | None = None):
    ...
    async def handler(
        ...
        for attempt in range(MAX_FALLBACK_ATTEMPTS):
            ...
            except (AcpError, PoolExhaustedError) as e:
