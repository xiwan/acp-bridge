...
echo "--- 同步调用 ---"
resp=$("$CLIENT" -a opencode "Reply with only the word ok" 2>&1)
...
echo ""
echo "--- 流式调用 ---"
resp=$("$CLIENT" --stream -a opencode "Reply with only the word ok" 2>&1)
...
echo ""
echo "--- 异步任务 ---"
ASYNC_SESSION="00000000-0000-0000-0000-b00000000002"
job_id=$("$CLIENT" --async -a opencode -s "$ASYNC_SESSION" "Reply with only the word ok" 2>&1 1>/dev/null | grep job_id | sed 's/job_id: //')
if [[ -n "$job_id" ]]; then
    echo "  等待任务完成 (10s)..."
    sleep 10
    resp=$("$CLIENT" --job-status "$job_id" 2>/dev/null)
    run_test "异步任务查询有结果" "ok\|OK\|completed\|opencode" "$resp"
else
    echo "❌ 异步任务未返回 job_id"
    ((FAIL++))
fi
