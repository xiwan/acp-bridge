...
```bash
export ACP_BRIDGE_URL=http://<bridge-ip>:8010
...
```bash
curl -X POST http://<bridge>:8010/jobs \
...
```bash
curl http://<bridge>:8010/jobs/<job_id> \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X POST http://<bridge>:8010/tools/invoke \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "tool": "message",
...
```bash
ACP_TOKEN=<token> bash test/test.sh http://127.0.0.1:8010
```
...
```bash
ACP_TOKEN=<token> bash test/test.sh http://127.0.0.1:8010 --only codex
```
